...
def main():
  # download and extract Flickr.tar.gz beforehand
  with open('cats') as f:
    cats = f.read().split('\n')
  ...
  for data_type in data_types:
    ...
    label_h = f_out.create_dataset(data_type + '_labels', (len(image_names), len(cats)), dtype=np.float32)
  ...
  f_out.create_dataset('mean', data = np.array([112.819626, 108.13823, 99.75903], dtype=np.float32))
...
if __name__ == "__main__":
  ...
  parser.add_argument('resize', choices=['224x224', 'original'])
